...
需要特别区分两种自驱动形态：模块 6 的 `/os_runtime goal` 是 turn-boundary assisted continuation，需要用户显式设置目标；完整元神自驱动应由后续常驻自治循环与全 turn 张力治理共同承担。常驻自治循环在 Agent profile 或 gateway/runtime 启动后自动在线，通过 Linz World 世界事件、时间 tick、内部反馈和生命状态变化唤醒张力场评估，不依赖用户每次输入 `/os_runtime`。全 turn 张力治理则要求普通 CLI/TUI/gateway 对话、世界事件、scheduled tick、工具反馈和 assistant response 都投影到 `os_runtime`，并在 turn 前/turn 后经过张力场观测、SelfPrompt 注入或行动裁判。该能力仍必须是配置 opt-in，并且第一版只开放低风险自主响应、草案、反思、证据和可观测状态；外部发布、工具副作用和世界市场交易仍由裁判、policy、授权 map 和审批边界控制。
...
| 阶段 3.5：常驻自治运行循环与全 turn 张力治理 | Agent/gateway 启动后按世界事件、时间 tick、普通对话和内部反馈自动唤醒/更新张力场 | 无需 `/os_runtime goal` 的低风险自主响应、草案、反思、ephemeral SelfPrompt 和 evidence | 中 |
...
5. `mode=autonomous_low_risk` 在本模块只保留为配置/状态枚举和 fail-closed 路径；真正的常驻自治循环、事件唤醒和无 `/os_runtime goal` 的自主响应由模块 6.5 实现。
6. 添加 CLI/slash 命令：
   - `/os_runtime status`
   - `/os_runtime passive`
...
### 模块 6.5：常驻 Autonomous Runtime Loop、全 Turn 张力治理与世界事件自响应

目标：在不依赖用户输入 `/os_runtime goal` 的情况下，让普通 Hermes Agent 以当前 profile 的 Linz World original spirit 身份运行，并让所有输入面都进入张力场治理。运行时既要根据世界事件、时间 tick、内部反馈、生命状态、张力场和行动势能自动唤醒，也要让普通 CLI/TUI/gateway 对话在 turn 前/turn 后经过事件投影、生命状态更新、张力场更新、行动势能评估、可选 SelfPrompt 注入和裁判记录。该模块是“完全元神自驱动”的入口，但第一版仍不允许默认高风险工具执行、world publish 或经济结算。

能力分层：

- Level 1 `identity-native`：普通 Agent 天然拥有 Linz World original spirit 身份，由模块 -1 保证。
- Level 2 `tension-observed`：所有普通对话、世界事件、tick、工具反馈和 assistant response 都进入 `os_runtime` 事件投影，更新 LifeState/Tension/ActionPotential/evidence。
- Level 3 `tension-driven`：所有 turn 前/turn 后可被张力场影响，包括 ephemeral SelfPrompt 注入、行动深度建议、是否继续、是否休眠、是否审批和是否拒绝外部行动。模块 6.5 交付 Level 2/3 的运行入口。

新增：

- `agent/os_runtime/autonomous_loop.py`
- `agent/os_runtime/autonomous_state.py`
- `agent/os_runtime/autonomous_scheduler.py`
- `agent/os_runtime/world_event_waker.py`
- `agent/os_runtime/autonomous_inbox.py`
- `agent/os_runtime/turn_hooks.py`
- `agent/os_runtime/self_prompt_injector.py`
- `agent/os_runtime/adapters/runtime_queue.py`
- `hermes_cli/os_runtime_autonomous.py`
- `tests/os_runtime/test_autonomous_loop.py`
- `tests/os_runtime/test_turn_hooks.py`
- `tests/os_runtime/test_world_event_waker.py`
- `tests/gateway/test_os_runtime_autonomous_loop.py`
...
1. 定义全局运行层级和配置门：
   - `identity_native` 由模块 -1 提供，不在本模块重复注册身份。
   - `apply_to_all_turns=false` 时，普通对话不进入全 turn hook，只保留模块 6 assisted 行为。
   - `apply_to_all_turns=true` 且 `inject_self_prompt=false` 时，只做张力观测和 evidence，不影响 prompt。
   - `inject_self_prompt=true` 时，只向当前轮 user context 注入 ephemeral SelfPrompt，不修改稳定 system prompt。
2. 定义 `AutonomousRuntimeState`：
   - `status`: `stopped | idle | sleeping | evaluating | acting | paused | failed`
   - `loop_id`
   - `profile_id`
   - `session_id`
   - `level`: `tension_observed | tension_driven`
   - `last_wake_reason`
   - `last_wake_event_id`
   - `last_turn_event_id`
   - `last_tick_at`
   ...
   - `last_action_summary`
   - `paused_reason`
3. 新建 `TurnTensionHook`：
   - 覆盖 CLI、TUI、gateway、Linz World MessageEvent、scheduled tick、assistant response、tool feedback 的统一入口。
   - `before_turn()` 将 incoming message 投影为 `human_request/world_event/runtime_tick`，构建 context/signals/life/tension/action/self_prompt。
   - `after_turn()` 将 assistant response、tool result、runtime feedback 投影回 `os_runtime`，更新 LifeState/TensionSet/ActionPotential/evidence。
   - hook 失败必须 fail-open 对主对话可用性、fail-closed 对外部副作用；不得让普通聊天崩溃。
4. 新建 `SelfPromptInjector`：
   - 根据最新 SelfPrompt、OpenSpace、TargetDirection 和 constraints 生成当前轮 ephemeral context。
   - 不修改 system prompt，不写长期 prompt cache 前缀。
   - 仅注入摘要、约束和证据引用，不暴露 token、raw payload 或 restricted audit content。
5. 新建 `AutonomousRuntimeLoop.run_once(wake_reason, event_ref=None)`：
   - 从 `autonomous_inbox` 或 `OSRuntimeEventRepository` 读取未处理世界事件/内部反馈。
   - 调用模块 1-5 的 context/signals/life/tension/action/self_prompt/intent/arbiter 链。
   - 在没有用户 goal 时，允许由张力场、生命状态和 action potential 共同决定是否生成自主 intent。
   - 把每次 wake 的输入、输出、裁判和停止原因写入 evidence。
6. 新建 `AutonomousScheduler`：
   - 在 gateway/runtime 启动时按配置启动。
   - 支持固定 tick、idle cooldown、hourly wake budget、pause/resume/stop。
   - 不创建无限无预算循环；每次 wake 最多 `max_turns_per_wake`。
7. 新建 `WorldEventWaker`：
   - 订阅 Linz World gateway adapter 已持久化的 world event。
   - `wsp.chat.message.sent`、需求/任务/订单/交付/治理等事件进入 autonomous inbox。
   - 重复事件只唤醒一次。
8. 自主行动第一版只允许：
   - `report_only`：记录观察、张力变化、建议和审批需求。
   - 低风险 `auto_execute`：生成自然语言回复草案、文档草案、内部反思、evidence package。
   - `require_approval`：生成用户可见审批请求。
   - 不允许默认工具执行、真实 world publish、外部消息发送、删除、支付或权限操作。
9. 如配置显式允许 `allow_world_publish=true`，world publish 仍必须同时通过：
   - BoYueArbiter
   - PolicyEngine
   - Linz event catalog
   - authorization map
   ...
   - evidence receipt
10. 提供观测命令：
   - `hermes os-runtime autonomous status`
   - `hermes os-runtime autonomous pause`
   - `hermes os-runtime autonomous resume`
   - `hermes os-runtime autonomous stop`
   - `hermes os-runtime autonomous tick`
   - `hermes os-runtime autonomous inbox`
   - `hermes os-runtime autonomous events`
   - `hermes os-runtime autonomous intents`

验收标准：

- 启用 `autonomous.enabled=true` 后，Agent/gateway 启动即可进入 idle/sleeping 常驻状态，不需要用户输入 `/os_runtime goal`。
...
- 注入一条合法 Linz World 消息事件后，runtime 自动 wake，完成 context -> signals -> life -> tension -> action potential -> self prompt -> intent -> arbitration 链路。
- 没有用户 goal 时，Agent 仍可基于张力场和生命状态生成低风险自主 intent。
- 默认配置下，世界事件只会产生回复草案、report 或审批请求，不会自动 `linz_publish`。
- tick wake 可以在无世界事件时触发内部反思、未完成张力检查或休眠决策。
- cooldown、wake budget、fatigue/restraint 和用户 pause/stop 能阻止无限自驱动。
- 每次 wake 都有可查询 evidence：wake reason、event id、LifeState、top tensions、ActionPotential、OpenIntent、ArbitrationResult、action summary、stop reason。
- `os_runtime.enabled=false` 或 `autonomous.enabled=false` 时，gateway/TUI/CLI 行为不变。
...
- AutonomousRuntimeLoop 的 wake budget、cooldown、tick wake、world event wake、pause/stop 和 fail-closed。
- TurnTensionHook 的普通对话 before_turn/after_turn 投影、状态更新和 fail-open 对话兼容性。
- SelfPromptInjector 的 ephemeral context 注入、system prompt 不变和敏感信息脱敏。
- WorldEventWaker 的 dedupe、inbox 入队、低风险 world event 自响应决策。
...
- `autonomous_low_risk` 模式不需要 `/os_runtime goal`，可由 world event 或 tick 自动 wake。
- `apply_to_all_turns=true` 时普通对话也会进入张力场事件投影和状态更新。
- `inject_self_prompt=true` 时普通对话使用 ephemeral SelfPrompt，但不改变 stable system prompt。
- 默认 autonomous world event 只生成草案/report/审批请求，不自动发布正式事件。
...
6. 用户可开启 `os_runtime.mode=autonomous_low_risk` 与 `autonomous.enabled=true`，Agent 在不输入 `/os_runtime goal` 的情况下按 world event、tick 和内部反馈自主 wake。
7. 用户可开启 `apply_to_all_turns=true`，让普通 CLI/TUI/gateway 对话也进入张力场观测、状态更新和 evidence。
8. 用户可开启 `inject_self_prompt=true`，让普通对话受到张力场生成的 ephemeral SelfPrompt 影响，同时稳定 system prompt 不变。
9. 每次自动推进、autonomous wake 或普通 turn 张力治理都有 tension interpretation、intent/arbitration 或观测 evidence。
10. 高风险工具和世界事件发布不会被默认自动执行。
11. 自驱动能被 pause/clear/interrupt/stop 停止，并受 cooldown、wake budget、fatigue/restraint 限制。
12. 不破坏现有 `/goal`、普通 CLI、gateway、TUI、工具调用、memory provider 行为。
13. LifeState、TensionSet、ActionPotential、SelfPrompt、OpenIntent、ArbitrationResult 覆盖白皮书 A-G 模块的必备字段，不再停留在旧的简化字段集。

MVP 不做：

- 不做无治理、无预算、无审批的完全自治外部执行。
- 不做完全自治世界市场交易。
- 不做经济结算。
...
5. 默认只读、被动观测先行。
6. 自驱动 continuation 和 autonomous wake 都必须有预算上限。
7. 常驻自治循环必须有 cooldown、wake budget、fatigue/restraint 抑制和用户 pause/stop 优先级。
8. 所有自动行动必须有 stop condition。
9. 工具副作用必须经过 arbiter/policy，并且 evidence 只能记录 `auto_execute`、`sandbox_execute`、`require_approval`、`report_only`、`reject` 五类主裁判结果。
10. 事件、意图、裁判、执行、证据必须可关联 trace。
11. 记忆回写必须分层，不能把所有事件塞入长期记忆。
12. 规则结晶必须有成熟度，不允许偶发事件直接升级硬规则。
13. 复用现有 plugin hook、kanban、delegate、memory、gateway/TUI，不复制基础设施。
14. `os_runtime` 中凡是叫 `context`、`store`、`memory`、`tools` 的文件都必须是 adapter，不得替代同名现有基础设施。
15. 所有路径必须 profile-aware，使用 `get_hermes_home()`；旧 `~/.linz-world` 只作为迁移来源，不把它扩散到 Hermes 运行期状态。
...
| 常驻循环失控 | tick 或 world event 持续唤醒导致后台自激 | wake budget、idle cooldown、dedupe、fatigue/restraint、pause/stop、per-wake max turns |
...
Linz World native identity/status/map/message projection
  ...
  -> AutonomousRuntimeLoop world-event/tick wake
...
```text
Hermes CLI/TUI/gateway 以 os_runtime.mode=autonomous_low_risk + autonomous.apply_to_all_turns=true 启动
用户直接发送普通消息，不输入 /os_runtime goal
TurnTensionHook.before_turn 将 human_request 投影为 os_runtime event
ContextAdapter/SignalInterpreter 生成 task context、agent context 和 signals
LifeStateSystem/TensionInterpreter/TensionFieldEngine 更新生命状态和张力网络
ActionPotentialEvaluator 计算行动深度；SelfPromptCompiler 生成 open_space/target_direction
若 inject_self_prompt=true，SelfPromptInjector 只向当前 user context 注入 ephemeral 摘要
Assistant 正常回复
TurnTensionHook.after_turn 将 assistant response 投影为反馈事件，并更新 evidence
普通对话可用性不受 hook 失败影响；外部副作用仍 fail-closed
```
...
```text
Hermes gateway/runtime 以 os_runtime.mode=autonomous_low_risk + autonomous.enabled=true 启动
AutonomousScheduler 进入 idle/sleeping，不需要用户输入 /os_runtime goal
Linz World NATS 订阅收到 wsp.chat.message.sent
WorldEventWaker 持久化并去重后把 event_ref 放入 autonomous inbox
AutonomousRuntimeLoop wake(reason=world_event)
ContextAdapter/SignalInterpreter 识别关系维护与低风险回复机会
LifeState 更新 social_hunger/silence_pressure/restraint
TensionInterpreter/TensionField 生成或增强 relationship/unsatisfied_response 张力
ActionPotentialEvaluator 判断值得生成回复草案但不值得直接发布
OpenIntentGenerator 生成 communicate/draft_reply intent
BoYueArbiter: report_only 或低风险 auto_execute(draft only)
EvidencePackage 记录 wake reason、event、生命状态、张力、势能、intent、裁判和草案
默认配置下不自动 linz_publish；若需要发布则进入 require_approval
AutonomousScheduler 根据 cooldown/fatigue/wake budget 休眠
```
