...
1. **设置**: 从仓库根目录根据当前系统运行先决条件脚本并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表: Windows 使用 `.specify/scripts/powershell/check-prerequisites.ps1 -Json`; macOS/Linux 使用 `.specify/scripts/bash/check-prerequisites.sh --json`。所有路径必须是绝对路径。对于参数中的单引号（如 "I'm Groot"），使用当前 shell 的转义语法（或尽可能使用双引号: "I'm Groot"）。
